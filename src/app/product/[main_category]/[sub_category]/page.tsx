...
async function getProductData(
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/api/product/${main_category}/${sub_category}/`,
      {
        next: { revalidate: 300 },
        cache: 'no-store'
      }
...
export async function generateMetadata({
  ...
  try {
    const productData = await getProductData(main_category, sub_category);
    ...
    <div className="min-h-screen bg-white" dir="rtl">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 mt-10 pb-16">
        ...
        <div className="text-right text-sm text-gray-600 mb-6">
            ...
            <Link href={`/product/${main_category}`} className="hover:text-[#246e72]">{productData?.main_category.name}</Link>
            <span className="mx-2">/</span>
